...
/** key={imageUrl} on parent remounts this subtree so zoom/pan reset without an effect */
function ImagePreviewBody({ imageUrl, title }: { imageUrl: string; title: string }) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-6xl w-full h-[90vh] p-0 overflow-hidden bg-black/95 border-zinc-800 flex flex-col">
                {imageUrl ? (
                    <ImagePreviewBody key={imageUrl} imageUrl={imageUrl} title={title} />
                ) : null}
            </DialogContent>
        </Dialog>
    );
