...
export default function SaveLoadModal({ onClose }: { onClose: () => void }) {
    ...
    const handleLoad = (design: SavedDesign) => {
        if (confirm(t('saveLoadModal.confirmLoad', { name: design.name }))) {
            loadDesign(design)
            onClose()
        }
    }
    ...
    const handleDelete = (id: string, e: React.MouseEvent) => {
        e.stopPropagation()
        if (confirm(t('saveLoadModal.confirmDelete'))) {
            deleteDesign(id)
        }
    }
                    ...
                    {mode === 'save' && (
                        <div className="flex flex-col gap-4">
                            <div className="p-4 bg-[#111] border border-[#333] rounded">
                                <h3 className="text-xs font-bold text-gray-500 uppercase mb-2">{t('saveLoadModal.currentDesign')}</h3>
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-xs text-gray-400">{t('saveLoadModal.designName')}</label>
                                <input
                                    type="text"
                                    value={saveName}
                                    onChange={e => setSaveName(e.target.value)}
                                    placeholder={t('saveLoadModal.namePlaceholder')}
