...
export function TimeBasedStats({ stats }: TimeBasedStatsProps) {
    const periods = [
        { label: "Aujourd'hui", key: 'today', icon: Activity, color: "text-blue-500" },
        { label: "Cette Semaine", key: 'week', icon: BarChart3, color: "text-purple-500" },
        { label: "Ce Mois", key: 'month', icon: CalendarDays, color: "text-luxury-gold" },
    ];
                    ...
                    return (
                        ...
                        >
                            <div className="flex items-center gap-2 mb-6 group-hover:translate-x-1 transition-transform">
                                ...
                                <h3 className="font-serif text-sm tracking-widest uppercase">{p.label}</h3>
                            </div>
                            <div className="space-y-4">
                                <div className="flex justify-between items-end border-b border-white/5 pb-2">
                                    <span className="text-[10px] uppercase tracking-widest text-muted-foreground">Nvx Projets</span>
                                    <span className="font-serif text-xl">{data.count}</span>
                                </div>
                                <div className="flex justify-between items-end border-b border-white/5 pb-2">
                                    <span className="text-[10px] uppercase tracking-widest text-muted-foreground">Volume</span>
                                    <span className="font-serif text-xl">${data.volume.toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between items-end">
                                    <span className="text-[10px] uppercase tracking-widest text-green-500/70 font-bold">Encaissements</span>
