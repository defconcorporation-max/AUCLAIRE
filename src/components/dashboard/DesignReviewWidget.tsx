        ...
        <Card className="glass-card">
            <CardHeader className="py-4 border-b border-white/5">
                <CardTitle className="text-lg font-serif tracking-wide flex items-center gap-2">
                    ...
                    {t('dashboard.designReviewTitle')}
                </CardTitle>
                <CardDescription className="text-[10px] uppercase tracking-widest text-luxury-gold/70">{t('dashboard.designReviewSubtitle')}</CardDescription>
            </CardHeader>
                ...
                <div className="divide-y divide-white/5 max-h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-luxury-gold/20">
                    {projects.length === 0 ? (
                        <div className="p-12 text-center text-muted-foreground">
                            <p className="text-xs uppercase tracking-widest">{t('dashboard.designReviewEmpty')}</p>
                        </div>
                    ) : (
                            ...
                            >
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className="text-[10px] text-muted-foreground uppercase tracking-tighter">
                                            {project.affiliate?.full_name || t('dashboard.affiliateDirect')}
                                        </span>
                                        <span className="w-1 h-1 rounded-full bg-white/10" />
                                        <span className="text-[10px] font-bold text-luxury-gold tabular-nums">
                                            ${(project.budget || 0).toLocaleString()} {t('dashboard.budgetLabel')}
                                        </span>
                                    </div>
                                </div>
                                <Button size="sm" variant="outline" className="h-8 border-luxury-gold/30 text-luxury-gold hover:bg-luxury-gold hover:text-black gap-2 text-[10px] uppercase tracking-widest font-bold" asChild>
                                    <Link to={`/dashboard/projects/${project.id}`}>
                                        {t('dashboard.designReviewCta')} <ChevronRight className="w-3 h-3" />
                                    </Link>
                                </Button>
                            </div>
                        ))
