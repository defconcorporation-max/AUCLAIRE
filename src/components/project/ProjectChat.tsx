...
export const ProjectChat: React.FC<ProjectChatProps> = ({ projectId, channel }) => {
    ...
    const mutation = useMutation({
        mutationFn: (text: string) => apiChats.send({
            ...
            sender_id: user?.id || '',
            sender_name: profile?.full_name || t('common.user'),
    ...
    if (isLoading) return <div className="p-4 text-center text-muted-foreground">{t('projectChat.loading')}</div>;

    const placeholder = channel === 'internal'
        ? t('projectChat.placeholderInternal')
        : t('projectChat.placeholderClient');
            ...
            >
                {messages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-muted-foreground opacity-50 space-y-2">
                        ...
                        <p className="text-sm italic">{t('projectChat.empty')}</p>
                    </div>
                ) : (
                    messages.map((msg) => {
