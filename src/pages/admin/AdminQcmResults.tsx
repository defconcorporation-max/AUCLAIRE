...
export default function AdminQcmResults() {
    ...
    const getWrongAnswersProcessus = (answers: Record<number, number>) => {
        return qcmProcessus.filter(q => answers[q.id] !== q.correctAnswer).map(q => ({
            ...
            given: q.options[answers[q.id]] || "Non répondu"
        }));
    };
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in zoom-in-95">
            <div className="flex justify-between items-start">
                <div>
                    <h1 className="text-4xl font-serif text-luxury-gold tracking-widest uppercase mb-2">QCM Academy</h1>
                    <p className="text-gray-400">Suivi des résultats et analyse des compétences de la force de vente.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="bg-white/5 border-white/10 backdrop-blur-md">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-gray-400 uppercase tracking-wider flex items-center gap-2">
                            <GraduationCap className="w-4 h-4 text-luxury-gold" /> Total Évaluations
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="bg-white/5 border-white/10 backdrop-blur-md">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-gray-400 uppercase tracking-wider flex items-center gap-2">
                            <BarChart2 className="w-4 h-4 text-luxury-gold" /> Moyenne Processus
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="bg-white/5 border-white/10 backdrop-blur-md">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-gray-400 uppercase tracking-wider flex items-center gap-2">
                            <Diamond className="w-4 h-4 text-luxury-gold" /> Moyenne Expertise
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-4xl font-bold text-white">
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden backdrop-blur-md">
                ...
                {loading ? (
                    <div className="p-12 text-center text-gray-400">Chargement des résultats...</div>
                                    ...
                                    >
                                        <div className="flex items-center gap-6">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500">{new Date(result.created_at).toLocaleString('fr-FR')}</p>
                                            </div>
                                        </div>

                                        <div className="flex items-center gap-8">
                                            <div className="text-center">
                                                <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-1">Processus</p>
                                            ...
                                            <div className="text-center">
                                                <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-1">Expertise</p>
                                    ...
                                    {isExpanded && (
                                        <div className="bg-black/40 p-6 border-t border-white/5 animate-in slide-in-from-top-2">
                                            <h4 className="text-luxury-gold font-serif mb-6 text-xl flex items-center gap-2">
                                                <AlertCircle className="w-5 h-5" /> Analyse des lacunes ({totalErrors} erreurs)
                                            </h4>

                                            {totalErrors === 0 ? (
                                                <div className="bg-green-500/10 border border-green-500/20 p-4 rounded-xl flex items-center gap-3">
                                                    <CheckCircle2 className="text-green-500 w-5 h-5" />
                                                    <span className="text-green-100">Excellente maîtrise. Aucune erreur commise.</span>
                                                </div>
                                            ) : (
                                                <div className="grid md:grid-cols-2 gap-8">
                                                    {wrongProc.length > 0 && (
                                                        <div className="space-y-4">
                                                            <h5 className="text-sm font-medium text-gray-300 uppercase tracking-widest flex items-center gap-2 border-b border-white/10 pb-2"><BookOpen className="w-4 h-4" /> Lacunes Processus</h5>
                                                            {wrongProc.map((err, i) => (
                                                                <div key={i} className="bg-white/5 p-4 rounded-lg border border-red-500/20">
                                                                    ...
                                                                    <div className="space-y-1 text-xs">
                                                                        <p className="text-red-400 break-words"><span className="text-gray-500 uppercase tracking-widest text-[10px] mr-2">A répondu:</span> {err.given}</p>
                                                                        <p className="text-green-400 break-words"><span className="text-gray-500 uppercase tracking-widest text-[10px] mr-2">Attendu:</span> {err.expected}</p>
                                                                    </div>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    ...
                                                    {wrongExp.length > 0 && (
                                                        <div className="space-y-4">
                                                            <h5 className="text-sm font-medium text-gray-300 uppercase tracking-widest flex items-center gap-2 border-b border-white/10 pb-2"><Diamond className="w-4 h-4" /> Lacunes Expertise</h5>
                                                            {wrongExp.map((err, i) => (
                                                                <div key={i} className="bg-white/5 p-4 rounded-lg border border-red-500/20">
                                                                    <p className="text-sm text-white mb-3">"{err.question}"</p>
                                                                    <div className="space-y-1 text-xs">
